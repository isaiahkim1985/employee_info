function insertData() {
	...
	if (confirm('등록을 진행하시겠습니까?')) {
		writeForm.action = "/firstTest/insert.do"
		writeForm.submit();
	}
}
...
function validateForm() {
	...
	// 이름
	if (writeForm.p_name.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("성명을 입력해주세요.").css("color", "red");
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		writeForm.p_name.focus();
		return false;
	} else if (writeForm.p_name.value.search(/[A-Za-z0-9]/g) >= 0) {
	...
	//주민등록번호
	if (writeForm.p_number.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("주민등록번호를 입력해주세요.").css("color", "red");
		writeForm.p_number.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	...
	function check_jumin() {
		 var ju = writeForm.p_number.value.substring(0,6);
		 var min = writeForm.p_number.value.substring(7,14);
		 var jumin = ju+min;
		 //주민등록 번호 13자리를 검사한다.
	...
	//소속회사
	if (writeForm.p_company.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("소속회사를 입력해주세요.").css("color", "red");
		writeForm.p_company.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	} else if (writeForm.p_company.value.search(/[A-Za-z0-9]/g) >= 0) {
		$("#warningSpan").text("한글로 입력해 주세요.").css("color", "red");
		writeForm.p_company.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	...
	//입사일
	if (writeForm.p_join.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("입사일을 입력해주세요.").css("color", "red");
		writeForm.p_join.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	}else{
		$("#warningSpan").text("");
	...
	//부서
	if (writeForm.p_dept.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("부서를 입력해주세요.").css("color", "red");
		writeForm.p_dept.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	}else{
		$("#warningSpan").text("");
	...
	//직위
	if (writeForm.p_position.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("직위를 입력해주세요.").css("color", "red");
		writeForm.p_position.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	}else{
		$("#warningSpan").text("");
	...
	//입대일 제대일
	//부서
	if (writeForm.pm_start.value.replace(/\s/g, "") != "" && writeForm.pm_end.value.replace(/\s/g, "") == "" || 
			writeForm.pm_start.value.replace(/\s/g, "") == "" && writeForm.pm_end.value.replace(/\s/g, "") != "") {
		$("#warningSpan").text("입대일과 제대일을 바르게 입력해주세요.").css("color", "red");
		writeForm.pm_start.focus();
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		return false;
	}else{
		$("#warningSpan").text("");
	...
	//전화
	var sCode = writeForm.p_phone.value.replace(/[^0-9]/g, '').substring(0,2);
	var areaCode = writeForm.p_phone.value.replace(/[^0-9]/g, '').substring(0,3);
	if(writeForm.p_phone.value.replace(/[^0-9]/g, '')!='' && sCode=='02'){
		if(writeForm.p_phone.value.replace(/[^0-9]/g, '').length < 9){
			$("#warningSpan").text("전화 번호를 바르게 입력해주세요.").css("color", "red");
			setTimeout(function(){$("#warningSpan").text("");}, 3000);
			writeForm.p_phone.focus();
			return false;
		}
	} else{
	...
	var frontNum = writeForm.p_cell.value.substring(1,4);
	if (writeForm.p_cell.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("휴대전화 번호를 입력해주세요.").css("color", "red");
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		writeForm.p_cell.focus();
		return false;
	...
	// 이메일
	if (writeForm.p_email.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("이메일을 입력해주세요.").css("color", "red");
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		writeForm.p_email.focus();
		return false;
	} else if(writeForm.p_email.value.search(/[0-9a-zA-Z][_0-9a-zA-Z-]*@[_0-9a-zA-Z-]+(\.[_0-9a-zA-Z-]+){1,2}$/)){
	...
	//주소
	if (writeForm.p_addr.value.replace(/\s/g, "") == "") {
		$("#warningSpan").text("주소를 입력해주세요.").css("color", "red");
		setTimeout(function(){$("#warningSpan").text("");}, 3000);
		writeForm.p_addr.focus();
		return false;
	} 
	else {
		$("#warningSpan").text("");
